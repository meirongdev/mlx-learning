...
Re-measured on **M5 (2026-06-30)** with `mlx-community/Qwen3.6-27B-4bit` (dense, **hybrid** Gated-DeltaNet/SSM + full attention, 64 layers): **~4.4 tok/s** at 512 and 1024 tokens — ~10× slower than the 35B-A3B MoE on the same box. Verified *not* swap-bound (0.31 GB page-ins during a full decode; model stays resident under the 26 GB GPU wired limit). It hits only ~39% of its ~11.4 tok/s bandwidth ceiling because the linear-attention layers use an unoptimized sequential MLX path. The old M2 Pro "10.6" is **not** comparable — it predates this 2026 hybrid-attention VLM checkpoint. ⚠️ **`nvidia/Qwen3.6-27B-NVFP4` cannot run on Apple Silicon**: it's an NVIDIA ModelOpt (BF16/F8_E4M3/U8) checkpoint for vLLM/TensorRT-LLM on Hopper/Blackwell CUDA — a different on-disk format from MLX-native NVFP4; no MLX runtime (omlx, mlx-lm, vllm-mlx) loads it, and no MLX-native NVFP4 build of any Qwen3.6-27B exists on HF. Full report: [`bench-results/m5-qwen3.6-27b-dense-4bit-20260630.md`](./bench-results/m5-qwen3.6-27b-dense-4bit-20260630.md).
